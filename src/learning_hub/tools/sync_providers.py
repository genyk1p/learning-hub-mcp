                ...
                if sync_cron_flag == "false":
                    ...
                    if updated is not None:
                        cron_note = (
                            ...
                            f"- Session: `--session isolated` (MUST be isolated, not main — "
                            f"main session leaks cron output into active user chats)\n"
                            f"- Use `--message` (not `--system-event`)\n"
                            f"- Use `--timeout-seconds 300`\n"
                            f"- Use `--announce --to <ADMIN_TG>` to deliver the sync summary "
                            f"to the admin's Telegram\n"
                            f"- Action: call `{TOOL_RUN_SYNC}()` to sync all active providers. "
                            f"If new grades or homeworks were synced, report them to the admin: "
                            f"list each new grade (subject, value, date, topic) and each new homework "
                            f"(subject, description, deadline)\n"
