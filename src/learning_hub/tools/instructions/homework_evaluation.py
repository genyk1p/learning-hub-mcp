from learning_hub.tools.config_vars import CFG_ISSUES_LOG, CFG_SUBJECT_RESPONSIBLES
...
4. Log in `{CFG_ISSUES_LOG}`: which homework could not be verified \
...
2. Forward to the adult responsible for this subject:
   - The student's solution (text/photo)
   - The recommended grade
   - A brief explanation of what is wrong
3. Determine the responsible adult from `{CFG_SUBJECT_RESPONSIBLES}` \
by subject (school + subject).
...
3. Log in `{CFG_ISSUES_LOG}` (date, context, error, status: open).
