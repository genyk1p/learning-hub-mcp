...
async def test_complete_ontime(session):
    """Homework completed before deadline -> DONE."""
    ...
    completed = await repo.complete(hw.id)
...
async def test_complete_overdue(session):
    """Homework completed after deadline -> OVERDUE."""
    subject_id = await _setup(session)
    repo = HomeworkRepository(session)
...
async def test_complete_idempotent_done(session):
    """Completing already-DONE homework returns it as-is."""
    subject_id = await _setup(session)
    repo = HomeworkRepository(session)
    ...
    await repo.complete(hw.id)
    result = await repo.complete(hw.id)
...
async def test_close_overdue_batch(session):
    """close_overdue closes all pending homeworks past deadline."""
    subject_id = await _setup(session)
    repo = HomeworkRepository(session)
